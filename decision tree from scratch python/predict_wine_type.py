...
dataFrame = pandas.read_csv("../datasets/WINE_DATA.csv")
# dataFrame = dataFrame.drop("id", axis = 1)
# dataFrameTrain = dataFrame[dataFrame.columns.tolist()[1: ] + dataFrame.columns.tolist()[0: 1]]
dataFrameTrain, dataFrameValidation = trainTestSplit(dataFrame, testSize = 0.25)
...
while accuracyTrain < 100:
    ...
    accuracyTrain = calculateAccuracy(decisionTreeTrainResults, dataFrameTrain.iloc[:, 0]) * 100
...
# def solution(test_user):
#     # Your code goes here
#     # decisio
#     i = 1
#     accuracyTrain = 0
#     # print(dataFrameTrain.iloc[:,0])
#     while accuracyTrain < 100:
#         startTime = time.time()
#         decisionTree = buildDecisionTree(dataFrameTrain, maxDepth = i)
#         buildingTime = time.time() - startTime
...
#     WINE_TYPE = solution( TEST_USER[1:])
#
